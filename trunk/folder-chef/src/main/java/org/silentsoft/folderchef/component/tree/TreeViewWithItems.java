...
//        	}
                ...
                while (change.next()) {
                    ...
                    if (change.wasRemoved()) {
                        for (int i = change.getRemovedSize() - 1; i >= 0; i--) {
                        	if (!treeItemChildren.isEmpty()) {
                        		int index = change.getFrom() + i;
                        		if (index >= 0 && index < treeItemChildren.size()) {
    ...
    private TreeItem<T> createNew(T value) {
    ...
    public T findNode(T node) {    	
    	for (int i=0, j=getItems().size(); i<j; i++) {
			T result = findNodeRecursively(getItems().get(i), node);
			if (result != null) {
				return result;
			}
		}
	...
	private T findNodeRecursively(T sourceNode, T targetNode) {
		if (sourceNode.equals(targetNode)) {
			return sourceNode;
		}
		
		for (int i=0, j=sourceNode.getChildren().size(); i<j; i++) {
			T child = sourceNode.getChildren().get(i);
			if (child.equals(targetNode)) {
				return child;
			} else {
				if (child.getChildren().size() > 0) {
					T result = findNodeRecursively(child, targetNode);
					if (result != null) {
						return result;
					}
				}
    ...
    public void insertIntoNode(TreeItem<T> node, T value) {
    	node.getChildren().add(createNew(value));
    }
    
    public void insertIntoSelectedItem(T value) {
    	getSelectionModel().getSelectedItem().getChildren().add(createNew(value));
    }
    
    public void changeSelectedItem(T value) {
    	getSelectionModel().getSelectedItem().setValue(value);
    }
}
